...
class listeEvenement extends personnaliser
{
    ...
    /**
     * Recupération des données par les setters année mois et jour la requête SQl retourne un tableau, on le découpe et le reforme afin de changer le code couleur
     * hex en rgb pour que le css adapte la couleur du texte
     * @return array
     */
    public function getEvenement(): array
    {
        $dateActuel = date('Y-m-d', strtotime($this->getYear() . '-' . $this->getMonth() . '-' . $this->getDay()));
        $reqjour = $this->db->prepare('select * from evenement join typeevenement t on evenement.Id_TypeEvenement = t.Id_TypeEvenement 
         where DATE (Datedebut_Evenement)<=? and  DATE (Datefin_Evenement)>=?   and Id_Client=?');
